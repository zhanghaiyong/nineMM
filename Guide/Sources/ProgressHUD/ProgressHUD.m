...
+ (void)allShow:(NSString *)status {

    [self shared].interaction = YES;
    [[self shared] hudMake:status image:nil spin:NO hide:NO];
}

+ (void)allShow:(NSString *)status Interaction:(BOOL)Interaction{
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	if (label.text != nil)
	{
		...
		hudHeight = labelRect.size.height + ((image.hidden) ? 30 : 88);
